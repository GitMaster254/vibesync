...
import { importAudioFiles } from '@/lib/import';
...
/**
 * Library page - Playlist management
 */
export default function Library() {
  ...
  const [tab, setTab] = useState<'playlists' | 'tracks'>('tracks');
  ...
  useEffect(() => {
    ...
    if (initialTab === 'tracks' || initialTab === 'playlists') {
      setTab(initialTab);
    }
  }, [searchParams]);
      ...
      <div className="container mx-auto max-w-2xl px-3 sm:px-4">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Your playlists and collections</p>
          </div>
            ...
            <>
              <input
                ...
                onChange={async (e) => {
                  ...
                  const target = e.currentTarget; // keep ref before awaiting
                  ...
                  try {
                    await importFilesWithWorker(
                      ...
                      (progress) => {
                        ...
                        // Capture errors during import, before they're cleared
                    ...
                    // Show warnings for duplicates after import completes
                ...
                ref={(el) => {
                  ...
                  if (el) {
                    try {
                      ...
                      // ignore failures setting non-standard attributes
                    }
                  }
                }}
                type="file"
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="min-w-[12rem]">
                  ...
                  <DropdownMenuItem
                    disabled={!canUseShowDirectoryPicker && !folderInputRef.current}
                    onClick={async () => {
                      if (canUseShowDirectoryPicker) {
                        try {
                          const dirHandle = await (window as unknown as { showDirectoryPicker: () => Promise<FileSystemDirectoryHandle> }).showDirectoryPicker();
                          const files: File[] = [];
                          type EntryTuple = [string, FileSystemFileHandle | FileSystemDirectoryHandle];
                          type DirWithEntries = { entries: () => AsyncIterableIterator<EntryTuple> };
                          const withEntries = dirHandle as unknown as DirWithEntries;
                          ...
                          await importAudioFiles(files, loadTracks);
                        } catch (err) {
                          if ((err as { name?: string })?.name !== 'AbortError') {
                            console.error(err);
                            toast.error('Folder access failed');
                          }
                        }
                      } else {
        ...
        <div className="mb-4 flex items-center gap-2">
          <Button variant={tab === 'tracks' ? 'default' : 'outline'} onClick={() => setTab('tracks')}>Tracks</Button>
          <Button variant={tab === 'playlists' ? 'default' : 'outline'} onClick={() => setTab('playlists')}>Playlists</Button>
        </div>

...
// Separate component for Queue Items to handle individual drag controls
function QueueItem({ 
  track, 
  actualIndex, 
  onPlay, 
  ...
  return (
    <Reorder.Item
      value={track}
      dragListener={false} // 👈 This disables dragging on the whole row
      dragControls={dragControls} // 👈 This connects dragging to our specific handle
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      whileDrag={{ scale: 1.02, backgroundColor: "rgba(255,255,255,0.05)" }}
      className="group flex items-center gap-4 p-3 rounded-2xl hover:bg-primary/5 transition-all select-none"
    >
      {/* Drag Handle - Only this triggers reordering */}
      <div 
        className="cursor-grab active:cursor-grabbing p-2 -ml-2 opacity-40 hover:opacity-100 transition-opacity"
        onPointerDown={(e) => dragControls.start(e)} // 👈 Manually start drag
      >
            ...
            >
              <div className="flex items-center justify-between mb-6 flex-shrink-0">
                <div className="flex gap-8">
                  <button onClick={() => setActivePanel('upNext')} className={cn('text-xs font-black uppercase tracking-[0.25em] pb-2 transition-colors', activePanel === 'upNext' ? 'text-foreground border-b-2 border-primary' : 'text-muted-foreground')}>UP NEXT</button>
                  <button onClick={() => setActivePanel('lyrics')} className={cn('text-xs font-black uppercase tracking-[0.25em] pb-2 transition-colors', activePanel === 'lyrics' ? 'text-foreground border-b-2 border-primary' : 'text-muted-foreground')}>LYRICS</button>
                </div>
                <button onClick={() => setActivePanel(null)} className="p-2 hover:bg-muted rounded-full transition-colors"><X className="h-5 w-5 text-muted-foreground" /></button>
              </div>
              ...
              <div className="flex-1 overflow-hidden">
                {activePanel === 'upNext' && (
                  <div className="flex flex-col h-full">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-[10px] font-black tracking-widest text-muted-foreground uppercase">Handle to Reorder</h3>
                      <Button variant="ghost" size="sm" className="text-[10px] font-bold text-muted-foreground hover:text-destructive" onClick={clearQueue}>CLEAR QUEUE</Button>
                    </div>
                ...
                {activePanel === 'lyrics' && (
                  <div className="h-full overflow-y-auto pr-2 custom-scrollbar text-lg font-medium leading-relaxed text-muted-foreground/80 whitespace-pre-line pb-10">
                    Lyrics synced with music coming soon...
                  </div>
                )}
              </div>
            </motion.div>

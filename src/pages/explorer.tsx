          ...
          <TabsContent value="genres" className="mt-4">
            <AnimatePresence mode="wait">
              {selectedGenre ? (
                ...
                <motion.div key="genres-grid" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
                  {emptyGenres ? (
                    ...
                    genres.map((genre) => {
                      ...
                      return (
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} key={genre.id} className="group relative aspect-square rounded-lg overflow-hidden bg-gradient-to-br from-muted to-muted-foreground/10 cursor-pointer" onClick={() => handleGenreClick(genre)}>
                          {genre.cover ? <img src={genre.cover} alt={genre.name} className="h-full w-full object-cover group-hover:scale-110 transition-transform" /> : <div className="h-full w-full bg-gradient-primary flex items-center justify-center"><IconComponent className="h-4 w-4 text-white/70" /></div>}
                          <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-all" />
                          <div className="absolute bottom-1 left-1 right-1"><h3 className="text-white font-medium text-xs truncate">{genre.name}</h3></div>
                        </motion.div>
                      );
                    })
                  )}
